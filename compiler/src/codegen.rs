...
impl Codegen {
    pub fn new(base: u16, resolver: &Resolver) -> Self {
        Codegen {
            ...
            tiles: resolver.tiles.iter().map(|(k, v)| (k.clone(), v.index)).collect(),
        }
    }
            ...
            match b {
                ...
                Byte::RelOffset(name) => {
                    ...
                    if diff < -128 || diff > 127 {
                        return Err(format!("Relative jump to '{}' out of range ({})", name, diff));
                    }
    ...
    fn ld_a_n(&mut self, n: u8) {
        self.emit(0x3E); self.emit(n);
    }
    /// LD A, (nn)  (absolute WRAM)
    fn ld_a_mem(&mut self, addr: u16) {
        self.emit(0xFA); self.emit_u16(addr);
    }
    /// LD (nn), A
    fn ld_mem_a(&mut self, addr: u16) {
        self.emit(0xEA); self.emit_u16(addr);
    }
    /// LD B, A
    ...
    /// AND B
    fn and_b(&mut self) { self.emit(0xA0); }
    /// OR B
    ...
    /// CPL  (complement A)
    fn cpl(&mut self) { self.emit(0x2F); }
    /// JP nn
    fn jp_nn(&mut self, name: &str) {
        self.emit(0xC3); self.emit_label_addr(name);
    }
    /// JP NZ, nn
    fn jp_nz(&mut self, name: &str) {
        self.emit(0xC2); self.emit_label_addr(name);
    }
    /// JP Z, nn
    fn jp_z(&mut self, name: &str) {
        self.emit(0xCA); self.emit_label_addr(name);
    }
    /// JP NC, nn
    fn jp_nc(&mut self, name: &str) {
        self.emit(0xD2); self.emit_label_addr(name);
    }
    /// JP C, nn
    fn jp_c(&mut self, name: &str) {
        self.emit(0xDA); self.emit_label_addr(name);
    }
    /// JR e (relative)
    fn jr(&mut self, name: &str) {
        self.emit(0x18); self.emit_rel_offset(name);
    }
    /// CALL nn
    fn call(&mut self, name: &str) {
        self.emit(0xCD); self.emit_label_addr(name);
    }
    /// RET
    pub fn ret(&mut self) { self.emit(0xC9); }
    /// RETI
    fn reti(&mut self) { self.emit(0xD9); }
    /// PUSH AF
    ...
    /// LD HL, nn
    fn ld_hl_n(&mut self, v: u16) { self.emit(0x21); self.emit_u16(v); }
    /// LD BC, nn
    fn ld_bc_n(&mut self, v: u16) { self.emit(0x01); self.emit_u16(v); }
    /// LD DE, nn
    fn ld_de_n(&mut self, v: u16) { self.emit(0x11); self.emit_u16(v); }
    /// LD (HL), A
    fn ld_hl_a(&mut self) { self.emit(0x77); }
    /// LD A, (HL)
    fn ld_a_hl(&mut self) { self.emit(0x7E); }
    /// LDI (HL), A  — store A at (HL) then INC HL
    fn ldi_hl_a(&mut self) { self.emit(0x22); }
    /// INC HL
    fn inc_hl(&mut self) { self.emit(0x23); }
    /// DEC BC
    fn dec_bc(&mut self) { self.emit(0x0B); }
    /// LD A, B
    fn ld_a_b2(&mut self) { self.emit(0x78); }
    /// OR C
    fn or_c(&mut self) { self.emit(0xB1); }
    /// LD (HL+), A  (same as LDI)
    fn ldi_hl_a2(&mut self) { self.emit(0x22); }
    /// NOP
    ...
    /// LD SP, nn
    fn ld_sp_n(&mut self, v: u16) { self.emit(0x31); self.emit_u16(v); }
    /// LD (FF00+n), A  — LDH store
    fn ldh_n_a(&mut self, offset: u8) { self.emit(0xE0); self.emit(offset); }
    /// LD A, (FF00+n)  — LDH load
    fn ldh_a_n(&mut self, offset: u8) { self.emit(0xF0); self.emit(offset); }
    /// LD (FF00+C), A
    fn ldh_c_a(&mut self) { self.emit(0xE2); }
    /// LD A, (BC)
    fn ld_a_bc(&mut self) { self.emit(0x0A); }
    /// LD A, L
    ...
    /// JR NZ, e
    fn jr_nz(&mut self, name: &str) { self.emit(0x20); self.emit_rel_offset(name); }
    /// JR Z, e
    fn jr_z(&mut self, name: &str) { self.emit(0x28); self.emit_rel_offset(name); }
    ...
    pub fn gen_expr(&mut self, expr: &Expr) -> Result<(), String> {
        match expr {
            ...
            Expr::Str(_, line) => {
                return Err(format!("Line {}: string expressions not supported in codegen", line));
            }
        }
        Ok(())
    }
    ...
    fn gen_simple_binop(&mut self, op: &BinOp, line: usize) -> Result<(), String> {
        match op {
            BinOp::Add => { self.add_a_b(); }
            BinOp::Sub => { self.sub_b(); }
            BinOp::Eq  => { self.gen_cmp_result(CmpKind::Eq); }
            BinOp::NotEq => { self.gen_cmp_result(CmpKind::NotEq); }
            BinOp::Lt  => { self.gen_cmp_result(CmpKind::Lt); }
            BinOp::LtEq => { self.gen_cmp_result(CmpKind::LtEq); }
            BinOp::Gt  => { self.gen_cmp_result(CmpKind::Gt); }
            BinOp::GtEq => { self.gen_cmp_result(CmpKind::GtEq); }
            BinOp::Div => {
    ...
    fn jr_nc(&mut self, name: &str) { self.emit(0x30); self.emit_rel_offset(name); }
    fn jr_c(&mut self, name: &str)  { self.emit(0x38); self.emit_rel_offset(name); }
    ...
    fn gen_builtin_call(&mut self, func: &str, args: &[Expr], line: usize) -> Result<(), String> {
        match func {
            ...
            other => {
                ...
                if !args.is_empty() {
                    return Err(format!("Line {}: user function args not yet supported in codegen", line));
                }
                self.call(other);
            }
        }
        ...
        match stmt {
            ...
            Stmt::Let(decl) => {
                ...
                if let Some(var) = self.vars.get(&decl.name).cloned() {
                    ...
                    return Err(format!("Line {}: undefined variable '{}'", decl.line, decl.name));
                }
            }
            ...
            Stmt::If { cond, then, elifs, else_, line } => {
    ...
    pub fn emit_builtins(&mut self, need_just_pressed: bool) -> Result<(), String> {
        ...
        self.sub_n(4);
        self.ld_b_a();
        self.ld_a_n(0x10); self.ldh_n_a(0x00);  // select buttons: FF00 = $10
        self.nop(); self.nop();                   // wait 2 cycles
        self.ldh_a_n(0x00);                       // read FF00
        self.jr(&done_lbl.clone());
        // Dpad
        self.place_label(&btn_lbl);
        self.ld_a_n(0x20); self.ldh_n_a(0x00);  // select dpad: FF00 = $20
        self.nop(); self.nop();
        self.ldh_a_n(0x00);
        ...
        self.ld_a_b(); self.cp_n(0); self.jr_z(&send.clone());
        self.place_label(&sloop);
        // SRL C
        self.emit(0xCB); self.emit(0x39);
        self.dec_b();
        self.jr_nz(&sloop);
        ...
        self.emit(0xE6); self.emit(0x01); // AND 1  (keep only bit 0)
        self.ret();
        ...
        for _ in 0..5 { self.emit(0x87); } // ADD A,A ×5
        self.ld_l_a();
    ...
    fn ld_a_h2(&mut self) { self.emit(0x7C); }
}

enum CmpKind { Eq, NotEq, Lt, LtEq, Gt, GtEq }
